...
Technical debt accumulates in long-lived software projects through expedient design decisions whose corrective effort grows over time. Static analysis tools can identify thousands of debt indicators in a mature codebase, but maintainers operating under limited time and contributor capacity rarely have evidence-based guidance on which modules to address first. This thesis develops a reproducible cross-project framework for predicting high-risk technical debt at the file level in open-source Apache Java projects, shifting the prediction target from debt detection toward consequence-oriented prioritization. The study draws on the Technical Debt Dataset v2.0, which integrates version control history from GitHub repositories, static analysis findings from SonarQube, and confirmed defect records from the Apache JIRA tracker for 33 Apache Java projects. After applying eligibility filters that require sufficient pre-snapshot history and post-snapshot observation activity, 22 projects and 12,449 file-level instances enter the analysis. High-risk labels are produced through a combined labeling approach that integrates six binary signals drawn from two independent source types: three signals from current static analysis evidence (severity-flagged issues, remediation effort, code smell counts) and three from historical maintenance activity (bug-fix commit frequency, code churn, contributor count). Signal weights are derived empirically from point-biserial correlations with a post-snapshot bug-fix surrogate, producing a label aligned with future maintenance burden rather than tool output. Twenty-seven features organized into five conceptual families (size and complexity, static debt indicators, historical change, co-change graph centrality, prior defect history) are computed strictly from information available at or before the snapshot date. Four supervised classifiers (logistic regression, random forest, XGBoost, LightGBM) are trained with class-balanced weighting and hyperparameter optimization through Optuna's tree-structured Parzen estimator over thirty trials per model under a precision-recall area-under-curve objective. Within-project 10-fold cross-validation establishes the optimistic performance bound, and leave-one-project-out validation with cosine-similarity-weighted training establishes the cross-project transfer capability. LightGBM achieves the strongest leave-one-project-out result with an F1 of 0.7261, a precision-recall area under curve of 0.8871, and a cost-effectiveness recall at the top 20 percent of files of 0.8242. The cost-effectiveness recall indicates that a maintainer reviewing only the top 20 percent of files ranked by predicted risk in an unseen project recovers approximately 82 percent of files that will become maintenance-intensive in the next six months. This represents a substantial improvement over uninformed inspection at the same budget. Feature family ablation identifies historical change and prior defect history as the load-bearing predictive dimensions, with neither static nor historical features alone reaching the full model's performance. The thesis contributes a defensible labeling approach that combines current and historical evidence, quantified evidence of practical prioritization value, cross-project generalization evidence across 22 projects, and a fully reproducible pipeline on an established benchmark.
...
| 4.5 Feature Importance Analysis | 64 |
| 4.6 Feature Family Ablation | 67 |
| 4.7 Comparison to Prior Work | 70 |
| **Chapter 5: Discussion** | 72 |
| 5.1 Answers to Research Questions | 72 |
| 5.2 Key Findings | 75 |
...
| 3.2 | Twenty-two eligible Apache Java projects with snapshot metadata | 31 |
| 3.3 | Six binary signals comprising the combined labeling approach | 35 |
| 3.4 | Theoretical and empirical signal weights with interpretation | 37 |
| 3.5 | Twenty-seven engineered features organized into five conceptual families | 43 |
| 4.1 | Per-project positive rates and signal breakdowns | 54 |
| 4.2 | Within-project ten-fold cross-validation results across four models | 58 |
| 4.3 | Leave-one-project-out cross-project validation results | 61 |
| 4.4 | Top-ten permutation-importance features for the best model | 65 |
| 4.5 | Feature family ablation: only-this-family and leave-out-family modes | 68 |
| 4.6 | Comparison with prior cross-project technical debt prediction studies | 70 |
| C.1 | Per-project leave-one-project-out results (22 projects, 4 models) | 102 |
| C.2 | Tuned hyperparameters for the four model families | 105 |
| C.3 | Empirical and theoretical weight comparison with interpretation | 106 |
| C.4 | Per-project basename retention and exclusion counts | 107 |
...
| Figure | Title | Page |
|---|---|---|
| 3.1 | Per-project positive rate of high-risk technical debt labels | 38 |
| 3.2 | Composition of the combined label across signal sources per project | 39 |
| 3.3 | Risk score distribution by label class | 39 |
| 3.4 | Feature Spearman correlation matrix with family boundaries | 45 |
| 4.1 | Per-project basename resolution status (kept and dropped) | 57 |
| 4.2 | Within-project F1 comparison across the four models | 59 |
| 4.3 | F1 versus threshold curve for the best model | 60 |
| 4.4 | Per-project leave-one-project-out F1 distribution | 63 |
| 4.5 | Per-project leave-one-project-out cost-effectiveness at 20 percent | 63 |
| 4.6 | Pooled receiver operating characteristic and precision-recall curves | 64 |
| 4.7 | SHAP feature importance for the best model (top 15) | 66 |
| 4.8 | Feature family ablation: F1 impact of removing each family | 69 |
...
This study is scoped to Apache Java open-source projects at the file level of granularity. The data substrate is the Technical Debt Dataset v2.0 (Lenarduzzi et al., 2019), which contains version control history, static analysis output, and issue tracker data for 33 Apache projects implemented in Java. Static analysis findings used as feature inputs and as components of the label are drawn from SonarQube. The labeling approach combines current static analysis evidence with historical maintenance signals from version control and issue tracker activity, and the labels reflect maintenance consequences observed in a six-month post-snapshot window.

The study's scope implies four boundaries on the interpretation of its results. First, the results are specific to the Apache ecosystem, which has a distinctive governance model emphasizing distributed contributor ownership, mailing-list-driven decision making, and release-train conventions. Projects governed differently, including corporate monorepos, single-maintainer hobby projects, and projects with continuous-deployment release cadences, may exhibit different feature distributions and label patterns. Second, the results are specific to the Java language and to the static analysis configuration applied by the dataset's curators. Other languages have different idioms, different tooling ecosystems, and different ratios of static-analyzable patterns to total code volume. Third, the file-level granularity is appropriate for the prioritization task addressed here but does not directly support method-level or commit-level analysis, both of which would require different feature engineering and a different label definition. Fourth, the temporal scope of the dataset means that the snapshots analyzed range from the early 2000s through the mid 2010s; predictive performance on projects with snapshots from the late 2010s or the 2020s has not been directly evaluated.

These boundaries are stated explicitly so that the practical claims made in this thesis, particularly the cost-effectiveness recall figures and the cross-project generalization estimates, can be interpreted within their proper domain of applicability.
...
Size and complexity metrics form the baseline of nearly all software quality prediction work. Nagappan et al. (2006) provided one of the most-cited empirical demonstrations that size and complexity metrics predict component failures across a large industrial codebase, and McCabe (1976) provided the theoretical underpinning for cyclomatic complexity. Campbell (2018), working at SonarSource, introduced cognitive complexity as a refinement that better tracks human-perceived comprehension difficulty by penalizing nesting and structural patterns that increase reading effort beyond what cyclomatic complexity captures. The current study uses both cyclomatic and cognitive complexity, together with lines of non-comment code, function count, and class count, as the size and complexity family.

Static debt indicators, drawn from SonarQube output in this study and in much of the related literature, include code smell counts, bug counts, total estimated remediation effort, issue density normalized by file size, and duplicated lines density. Tsoukalas et al. (2020) used a similar set as feature inputs, distinguishing carefully between using SonarQube output as features and using SonarQube severity classifications as labels. The current study follows this distinction, using static debt indicators as features but using only severity flags as one of six signals in the combined labeling approach. Fowler (1999) provided the foundational framing for code smells and duplication as maintenance risk indicators, motivating duplicated lines density as a feature in subsequent quantitative studies.

Historical change metrics capture the evolution of each file through version control history. Kamei et al. (2013) used churn, commit count, file age, and contributor count among other features for just-in-time defect prediction. Hassan (2009) demonstrated the predictive value of recent activity windows, motivating this study's inclusion of 90-day and 30-day windows alongside lifetime metrics. The relationship between contributor count and quality is more nuanced than a simple monotonic increase or decrease implies. Bird et al. (2011) showed that the share of changes made by the dominant contributor, captured by the ownership ratio used in the current study, captures coordination risk in a way that contributor count alone does not. Mockus and Votta (2000) established commit message classification as a means of distinguishing maintenance types, underpinning the bug-fix activity signals used in both the labeling approach and the prior defect feature family.

Co-change graph features represent a more recent addition to the feature engineering toolkit. The premise is that file-level metrics, however richly computed, miss the relational structure that emerges when files change together over time. D'Ambros et al. (2012) demonstrated that graph centrality measures, including degree, weighted PageRank, and betweenness centrality on co-change graphs, capture architectural fragility beyond what individual file metrics convey. Files that sit at the center of dense co-change neighborhoods are structurally fragile, and changes to them tend to ripple outward to many other files. Ethari et al. (2025) added the entropy refinement described in the previous section.

Prior defect history features capture the principle, established empirically across decades of defect prediction work, that past defects predict future defects. Hassan (2009) and Kamei et al. (2013) both demonstrated the predictive value of prior bug-fix activity. The current study extends this family with JIRA-confirmed bug records following Falessi et al. (2020), who reported that officially confirmed and triaged defect tickets provide stronger predictive evidence than keyword-based commit identification alone. The current study includes both: keyword-derived bug-fix commit counts and JIRA-confirmed bug counts and blocker flags, retaining the advantages of both sources.
...
Applying these criteria to the 33 projects retains 22 projects in the analysis corpus. Nine projects were excluded for failing one or both thresholds, predominantly the post-snapshot threshold. Table 3.2 summarizes the 22 retained projects with their snapshot dates and the pre and post snapshot commit counts that satisfy the eligibility criteria.

**Table 3.2.** Twenty-two eligible Apache Java projects with snapshot metadata.
...
Table 3.3 specifies the six binary signals that compose the combined label.

**Table 3.3.** Six binary signals comprising the combined labeling approach.
...
The 75th percentile threshold is computed per project. For each signal, files above the project's 75th percentile on that dimension are flagged. This percentile-based design follows Kamei et al. (2013) and has two advantages. It identifies files that are genuinely elevated on each dimension within their own project context, rather than files that are merely above some global average that may be dominated by atypically large or active projects. And it ensures comparability across projects of different sizes and activity levels: a file in a small project flagged S5 has churn that is high relative to that project, just as a file in a large project flagged S5 has churn that is high relative to that project.
...
The weight derivation procedure computes the point-biserial correlation between each binary signal and the post-snapshot bug-fix count, takes the absolute value, normalizes the six correlations to sum to one, and uses the resulting normalized values as the signal weights. Where the empirical weights and a theoretical baseline derived from prior literature differ by more than 0.05 on any signal, the empirical weights are retained as the final weights. The theoretical baseline was constructed from Kamei et al. (2013) and Tsoukalas et al. (2020), with severity assigned the largest theoretical weight (0.30) on the prior intuition that severity flags represent the strongest tool-level evidence of risk.

Table 3.4 reports the theoretical and empirical weights together with an interpretation of the divergence.

**Table 3.4.** Theoretical and empirical signal weights with interpretation. Empirical weights derived through point-biserial correlation with a post-snapshot bug-fix surrogate on the 22-project corpus.

| Signal | Theoretical | Empirical | Chosen | Interpretation |
|---|---|---|---|---|
| S1 (Severity) | 0.30 | 0.15 | 0.15 | Severity flags alone correlate substantially less strongly with future bug-fix activity than the Kamei (2013) baseline anticipated. Severity is informative but not dominant. |
| S2 (Debt burden) | 0.20 | 0.15 | 0.15 | Modestly weaker than theoretical, but close to the baseline. |
| S3 (Code smells) | 0.05 | 0.16 | 0.16 | Substantially stronger than theoretical. The volume of code smell findings is a stronger empirical predictor than literature anticipated. |
| S4 (Bug-fix activity) | 0.25 | 0.23 | 0.23 | Near-theoretical. Prior bug-fix activity is a reliable predictor of future bug-fix activity. |
| S5 (Code churn) | 0.15 | 0.16 | 0.16 | Confirmed near-theoretical. |
| S6 (Contributors) | 0.05 | 0.15 | 0.15 | Substantially stronger than theoretical. Distinct contributor count is a stronger predictor in the Apache distributed ownership context than the baseline anticipated. |
...
Figure 3.1 shows the per-project positive rate. The rate varies from 7.89 percent (httpclient) to 26.67 percent (daemon), with a corpus mean near 15 percent. Variation across projects is expected and reflects differences in project size, age, and maintenance dynamics.

![Per-project positive rate of high-risk technical debt labels.](../results/figures/fig_01_positive_rates.png)

**Figure 3.1.** Per-project positive rate of high-risk technical debt labels. Each bar represents one project; colors distinguish projects with positive rates below 15 percent (green) from those above (orange).

Figure 3.2 decomposes the labels by signal-source composition. For each project, the figure shows how many files were labeled high-risk through (a) static signals alone, (b) historical signals alone, (c) both static and historical signals jointly, and (d) the negative class. The dominant pattern across most projects is co-occurrence of static and historical signals, validating the combined labeling premise that the two source types provide complementary evidence rather than redundant evidence.

![Composition of the combined label across signal sources per project.](../results/figures/fig_02_label_signal_breakdown.png)

**Figure 3.2.** Composition of the combined label across signal sources per project. The stacked bars decompose each project's labels by which signal sources triggered the high-risk flag.

Figure 3.3 shows the risk score distribution by label class. The bimodal pattern visible in the negative class reflects the percentile-based per-signal thresholding, with most negative files clustering at low scores and a smaller cluster near the 0.50 threshold. The positive class concentrates above the threshold by construction.

![Risk score distribution by label class.](../results/figures/fig_03_risk_score_distribution.png)

**Figure 3.3.** Risk score distribution by label class. The dashed vertical line indicates the 0.50 threshold separating the negative and positive classes.
...
This family captures the structural scale and intrinsic complexity of each file. Nagappan et al. (2006) established that size and complexity metrics predict component failures across a large industrial codebase, and McCabe (1976) provided the theoretical underpinning for cyclomatic complexity. The five features in this family are:

- **ncloc**: lines of non-comment code, drawn from SonarQube's measurement of executable code volume.
...
- **cognitive_complexity**: cognitive complexity (Campbell, 2018), a SonarQube measure that penalizes nesting and structural patterns increasing reading effort beyond what cyclomatic complexity captures.
- **functions**: method count.
- **classes**: class count.

### 3.4.3 Family 2: Static Debt Indicators

This family captures current static analysis evidence of debt at the snapshot date. Tsoukalas et al. (2020) demonstrated that SonarQube-derived quality metrics effectively identify debt-prone modules. The five features in this family are:

- **n_code_smells**: count of CODE_SMELL-typed SonarQube issues. These are maintainability violations distinct from defects.
- **n_bugs**: count of BUG-typed SonarQube issues regardless of severity.
- **total_debt_minutes**: SonarQube's estimated remediation effort summed across all issues on the file, in minutes.
- **issue_density**: total issue count normalized by ncloc, capturing size-adjusted debt intensity.
- **duplicated_lines_density**: percentage of duplicated lines, drawing on Fowler's (1999) framing of duplication as a propagation risk for fixes.
...
This family captures the evolution and contribution patterns of each file through version control history. Kamei et al. (2013) showed that process history outperforms static metrics for forward-looking risk estimation. Hassan (2009) demonstrated the predictive value of recent activity windows alongside lifetime metrics. Bird et al. (2011) highlighted the role of ownership concentration in coordination risk. The eight features in this family are:

- **total_commits_pre**: total number of commits touching the file before *t*.
- **code_churn_pre**: lifetime sum of lines added and lines removed for the file.
- **recent_churn_90d**: churn restricted to the 90 days before *t*, capturing whether the file is a current hotspot.
- **commit_frequency_30d**: commits in the 30 days before *t*, capturing very recent activity intensity.
- **file_age_days**: days between the file's first commit and *t*.
- **days_since_last_change**: days between the file's most recent commit and *t*, capturing whether the file is actively maintained or quiescent.
- **contributor_count**: distinct author count before *t*.
- **ownership_ratio**: the dominant contributor's commit share, computed as the maximum single-author commit count divided by total_commits_pre.

### 3.4.5 Family 4: Co-Change Graph Centrality

This family captures the architectural position of each file in the co-change graph, where edges connect files that have been modified in the same commits. D'Ambros et al. (2012) and related mining-software-repositories work have shown that graph and network-style metrics derived from co-change relations capture architectural fragility beyond what individual file metrics convey. Ethari et al. (2025) showed that co-change graph entropy further improves defect classification. The four features in this family are:

- **cocg_degree**: number of distinct files with which this file has co-changed.
- **cocg_pagerank**: weighted PageRank score, capturing recursive importance under the assumption that co-changing with central files makes a file more central.
- **cocg_betweenness**: betweenness centrality, identifying files that sit on many shortest paths between other files in the co-change graph.
- **cocg_entropy**: Shannon entropy over the file's neighbor edge weights (Ethari et al., 2025). High entropy indicates that the file co-changes with many partners in unpredictable contexts; low entropy indicates a tight, predictable co-change cluster.

### 3.4.6 Family 5: Prior Defect History

This family captures the accumulated defect history of each file. Hassan (2009) established that past defects predict future defects. Falessi et al. (2020) showed that JIRA-confirmed bugs provide stronger predictive evidence than keyword-based commit identification alone. Both signals are included to capture both the volume and the confirmed-severity aspects of historical defect activity. The five features in this family are:

- **bugfix_commits_pre**: lifetime count of bug-fix commits affecting the file, identified through a regular expression pattern over commit messages following the convention of Mockus and Votta (2000) and Fischer et al. (2003).
- **bugfix_commits_90d**: bug-fix commits in the 90 days before *t*.
- **bug_density_pre**: bugfix_commits_pre normalized by total_commits_pre, capturing what fraction of the file's commit history was corrective.
- **n_jira_bugs_pre**: count of JIRA-confirmed bug records linked to the file before *t*.
- **jira_blocker_flag**: binary flag indicating whether any linked JIRA bug carried a Blocker or Critical priority.

### 3.4.7 Full Feature Catalog

Table 3.5 lists all 27 features together with their family, source, supporting literature, and rationale for inclusion.

**Table 3.5.** Twenty-seven engineered features organized into five conceptual families.
...
**Random forest** is an ensemble of decision trees trained through bagging. Random forests handle non-linear interactions and feature interactions naturally and are widely used as strong baselines for tabular classification problems in software engineering (Tsoukalas et al., 2020).

**XGBoost** (Chen & Guestrin, 2016) is a gradient-boosted decision tree algorithm that builds trees sequentially, each tree correcting the residual errors of its predecessors. XGBoost has been consistently competitive on tabular classification problems across software engineering and machine learning benchmarks.

**LightGBM** (Ke et al., 2017) is a histogram-based gradient boosting algorithm that uses leaf-wise tree growth and gradient-based one-side sampling for efficient handling of large feature spaces and class-imbalanced datasets.
...
Hyperparameters were tuned through Optuna (Akiba et al., 2019) using its tree-structured Parzen estimator sampler over thirty trials per model, with an inner five-fold stratified cross-validation disjoint from the outer evaluation fold. The optimization objective was the precision-recall area under curve (PR-AUC), chosen because it is the area-under-curve metric most appropriate to imbalanced classification problems (Saito & Rehmsmeier, 2015). Logistic regression's small hyperparameter space was searched through grid search over five candidate inverse regularization strengths rather than through Optuna, reflecting the fact that grid search is exhaustive over five candidate values whereas Optuna's sequential exploration would not produce improvement.

### 3.5.3 Classification Threshold

The classification threshold was fixed at 0.5 for all four models. Preliminary investigations of per-fold threshold optimization showed no consistent improvement for tree ensembles trained with balanced class weights at this positive rate, and the standard threshold is retained for simplicity and interpretability. Reporting results at the standard threshold also facilitates comparison with prior cross-project defect and technical debt prediction work, much of which uses the standard threshold.
...
Within-project validation uses stratified 10-fold cross-validation on the pooled corpus. Files are randomly partitioned into ten folds, stratified to maintain the per-fold positive rate. Each fold serves once as the test fold while the remaining nine folds form the training set. Models are refit on each training partition and evaluated on the corresponding test partition. Within-project results establish an upper performance bound: because files from the same project can appear in both training and test folds (different files, same project), the model can exploit project-specific patterns. This produces optimistic but bounded estimates useful for model selection and comparison.
...
**F1** is the primary classification metric at threshold 0.5. F1 balances precision and recall and is widely used in defect and technical debt prediction literature.

**ROC-AUC** is the area under the receiver operating characteristic curve. ROC-AUC measures ranking quality across all possible thresholds and is informative about a model's ability to separate positive from negative instances regardless of the threshold chosen for binary classification.

**PR-AUC** is the area under the precision-recall curve. PR-AUC is more informative than ROC-AUC under class imbalance because it focuses on the positive class and does not reward correct negative classifications, which dominate at the 17 percent positive rate observed here (Saito & Rehmsmeier, 2015).

**CE@20** is the cost-effectiveness recall at the top 20 percent of files, an inspection-budget metric directly aligned with the practical prioritization task. CE@20 measures the fraction of truly high-risk files that a maintainer would recover by inspecting only the top 20 percent of files ranked by the model's predicted risk score. A CE@20 of 0.8242 means that by reviewing only 20 percent of files, a maintainer recovers 82.4 percent of files that will become maintenance-intensive in the next six months. CE@20 is equivalent to recall at 20 percent recall used in Kamei et al. (2013) and provides the most direct measurement of practical prioritization value among the four metrics reported.
...
The three tree ensembles cluster tightly together, with within-project F1 values in the 0.858 to 0.869 range. Logistic regression's lower F1 (0.733) reflects its inability to capture non-linear interactions among features. LightGBM achieves the highest within-project F1 by a narrow margin, with XGBoost a close second and random forest third. The ROC-AUC values for the three tree ensembles all exceed 0.985, indicating that the models separate positive from negative instances near-perfectly at some threshold; the F1 values at the standard 0.5 threshold are correspondingly high. Figure 4.2 visualizes the comparison.
...
The cost-effectiveness recall result deserves direct attention. LightGBM's CE@20 of 0.8242 means that a maintainer who applies the model to a previously unseen Apache Java project, ranks all files by their predicted risk score, and inspects only the top 20 percent of the ranked list would recover approximately 82.4 percent of files that will become maintenance-intensive in the six months following the snapshot. The corpus's 17 percent positive rate implies that random inspection of the same 20 percent budget would recover approximately 17 percent of high-risk files. The model therefore delivers approximately 4.8 times the recovery rate of random inspection under an identical inspection budget. This margin is the central practical finding of the thesis: it converts the long, undifferentiated list of static analysis findings into a ranked work plan that concentrates expected payoff into a small fraction of the codebase.

The within-project to LOPO generalization gap is 0.143 (within-project F1 0.869 minus LOPO F1 0.726). This gap is within the normal range reported by Herbold et al. (2018), who placed typical cross-project F1 between 0.25 and 0.45 with corresponding within-project values commonly 0.15 to 0.30 higher. The current study's gap of 0.143 sits at the favorable end of this range, indicating that the model learns patterns that transfer reasonably well across project boundaries rather than overfitting to project-specific artifacts.
...
Feature importance was analyzed through two complementary methods: SHAP (Shapley Additive Explanations) on the best model and permutation importance across all four models.
...
Table 4.4 reports the top ten features by permutation importance for the best model. Permutation importance measures the drop in PR-AUC when each feature is independently permuted on the test set, providing a complementary view that does not depend on the model's internal feature attribution.

**Table 4.4.** Top-ten permutation-importance features for the best model. Values are mean drops in precision-recall area under curve when the feature is permuted on the test set.
...
Third, the combined labeling approach used in this study avoids the tautological dependency that severity-only labels face (Schutz & Plosch, 2023). The empirical weight derivation reported in Chapter 3 showed that severity flags alone are weaker predictors of future bug-fix activity than literature anticipated, supporting the design choice to combine signals rather than rely on severity-based labels.
...
This operational definition addresses two failure modes of single-source labeling. The static-only failure mode, in which models learn to reconstruct static analysis tool output rather than predict real maintenance burden (Schutz & Plosch, 2023), is avoided by requiring corroborating evidence from independent historical signals. The historical-only failure mode, in which models conflate past problems with future risk without accounting for current structural state, is avoided by requiring corroborating evidence from current static evidence. The 15.22 percent corpus-level positive rate confirms that the label identifies a genuinely rare high-risk subset rather than flagging all files indiscriminately.
...
The generalization gap between within-project and LOPO F1 is 0.143 (0.869 minus 0.726), within the normal range reported by Herbold et al. (2018) for cross-project defect and technical debt prediction. The gap sits at the favorable end of this range, indicating that the model learns transferable rather than project-specific patterns. The cross-project F1 of 0.7261 is favorable relative to the cross-project performance characterizations reported in Tsoukalas et al. (2020, 2022), a difference plausibly attributable to the combined labeling approach, the broader feature representation, and the similarity-weighted training regime. Direct head-to-head numerical comparison is constrained by differences in label definition and dataset version, and is discussed further in Section 4.7.

On the question of which algorithm performs best, the answer is nuanced. LightGBM is the formal best by LOPO F1, but the difference between LightGBM and XGBoost is small enough that the two should be considered substantively tied. Random forest is slightly behind on F1 but achieves the highest LOPO CE@20 (0.8299), reflecting that random forest's calibration produces slightly stronger top-of-ranking concentration even when its overall classification accuracy is marginally below LightGBM's. The practical recommendation is that either LightGBM or XGBoost should be the default choice; both substantially outperform random forest, and all three substantially outperform logistic regression.
...
**Operationalization of high-risk technical debt.** The combined labeling approach operationalizes high-risk technical debt as a binary indicator derived from six measurable signals at a fixed snapshot, weighted empirically against a post-snapshot bug-fix surrogate. This operationalization aligns the label with future maintenance consequences, the central conceptual target of the consequence-oriented framing. Schutz and Plosch (2023) used a comparable approach of correlating static analysis findings with actual failure evidence to construct ground truth, supporting the methodological precedent for combining static and outcome-based signals. The combined label is most directly comparable to maintenance-outcome-based labels in the just-in-time defect prediction literature (Kamei et al., 2013; Hassan, 2009), with the addition of current static debt evidence as a complementary signal.
...
**Cost-effectiveness recall as a practical metric.** CE@20 directly measures the fraction of high-risk files recovered at a fixed inspection budget, providing a practically interpretable metric of prioritization value. The choice of 20 percent as the inspection budget is conventional in just-in-time defect prediction literature (Kamei et al., 2013) and represents a reasonable approximation of what a small open-source team can review during a release-hardening cycle. Different budgets (10 percent, 30 percent) would produce different CE numbers, and the relationship between budget and recall is not linear, so the practical interpretation should be anchored to the specific inspection capacity available in any given deployment context.
...
The empirical work was conducted on the Technical Debt Dataset v2.0 (Lenarduzzi et al., 2019), integrating version control history from GitHub, static analysis findings from SonarQube, and confirmed defect records from the Apache JIRA tracker. After applying eligibility filters requiring sufficient pre-snapshot history and post-snapshot activity, the analysis corpus comprises 22 Apache Java projects and 12,449 file-level instances at a 17.08 percent positive rate. Four supervised classifiers (logistic regression, random forest, XGBoost, LightGBM) were trained with class-balanced weighting and hyperparameter optimization through Optuna's tree-structured Parzen estimator, and evaluated under both within-project ten-fold cross-validation and leave-one-project-out cross-project validation.

The headline empirical results are as follows. LightGBM achieved the strongest cross-project performance with a leave-one-project-out F1 of 0.7261, a precision-recall area under curve of 0.8871, a receiver operating characteristic area under curve of 0.9679, and a cost-effectiveness recall at the top 20 percent of files of 0.8242. Within-project ten-fold cross-validation produced an F1 of 0.8686, establishing the optimistic performance bound. The within-project to leave-one-project-out generalization gap of 0.143 sits within the normal range reported by Herbold et al. (2018), confirming that the model learns patterns transferable across project boundaries. The cross-project F1 of 0.7261 compares favorably with prior cross-project characterizations on the Technical Debt Dataset reported by Tsoukalas et al. (2020, 2022), while differences in label definition and dataset version constrain direct head-to-head numerical comparison.
...
In addition to the headline numbers, the study contributes a defensible methodology for constructing labels aligned with future maintenance consequences rather than tool output, cross-project empirical evidence on which feature family combinations transfer reliably (with historical change and prior defect history identified as the load-bearing dimensions), and a fully reproducible pipeline on the established Technical Debt Dataset v2.0 benchmark.
...
Bird, C., Nagappan, N., Murphy, B., Gall, H., & Devanbu, P. (2011). Don't touch my code! Examining the effects of ownership on software quality. *Proceedings of the 19th ACM SIGSOFT Symposium and the 13th European Conference on Foundations of Software Engineering*, 4–14. https://doi.org/10.1145/2025113.2025119

Breiman, L. (2001). Random forests. *Machine Learning*, 45(1), 5–32. https://doi.org/10.1023/A:1010933404324

Campbell, G. A. (2018). *Cognitive complexity: A new way of measuring understandability* (SonarSource Technical Whitepaper). SonarSource S.A.

Chen, T., & Guestrin, C. (2016). XGBoost: A scalable tree boosting system. *Proceedings of the 22nd ACM SIGKDD International Conference on Knowledge Discovery and Data Mining*, 785–794. https://doi.org/10.1145/2939672.2939785

Codabux, Z., Williams, B. J., Bradshaw, G. L., & Cantor, M. (2020). An empirical assessment of technical debt practices in industry. *Journal of Software: Evolution and Process*, 32(11), e2287. https://doi.org/10.1002/smr.2287

Consortium for Information and Software Quality. (2022). *The cost of poor software quality in the US: A 2022 report*. CISQ.
...
D'Ambros, M., Lanza, M., & Robbes, R. (2012). Evaluating defect prediction approaches: A benchmark and an extensive comparison. *Empirical Software Engineering*, 17(4-5), 531–577. https://doi.org/10.1007/s10664-011-9173-9

Ethari, H., Kumar, A., Bhardwaj, M., & Agarwal, S. (2025). *Co-change graph entropy: A new process metric for defect prediction* (arXiv:2504.18511). arXiv. https://arxiv.org/abs/2504.18511

Falessi, D., Russo, B., & Mullen, K. (2020). What if a bug has a different origin? Making sense of bugs without an explicit bug introducing change. *Proceedings of the 14th ACM/IEEE International Symposium on Empirical Software Engineering and Measurement*, 1–6. https://doi.org/10.1145/3382494.3422161

Fischer, M., Pinzger, M., & Gall, H. (2003). Populating a release history database from version control and bug tracking systems. *Proceedings of the 19th International Conference on Software Maintenance*, 23–32. https://doi.org/10.1109/ICSM.2003.1235403

Fowler, M. (1999). *Refactoring: Improving the design of existing code*. Addison-Wesley.

Hassan, A. E. (2009). Predicting faults using the complexity of code changes. *Proceedings of the 31st International Conference on Software Engineering*, 78–88. https://doi.org/10.1109/ICSE.2009.5070510
...
Tsoukalas, D., Mittas, N., Chatzigeorgiou, A., Kehagias, D., Ampatzoglou, A., Amasaki, S., & Angelis, L. (2022). Machine learning for technical debt identification. *IEEE Transactions on Software Engineering*, 48(12), 4892–4906. https://doi.org/10.1109/TSE.2021.3129355
...
Lenarduzzi, V., Saarimaki, N., & Taibi, D. (2019). The Technical Debt Dataset. *Proceedings of the 15th International Conference on Predictive Models and Data Analytics in Software Engineering*, 2–11. https://doi.org/10.1145/3345629.3345630

Liu, Z., Huang, Q., Xia, X., Shihab, E., Lo, D., & Li, S. (2018). SATD detector: A text-mining-based self-admitted technical debt detection tool. *Proceedings of the 40th International Conference on Software Engineering Companion*, 9–12. https://doi.org/10.1145/3183440.3183478

Lundberg, S. M., & Lee, S.-I. (2017). A unified approach to interpreting model predictions. *Advances in Neural Information Processing Systems*, 30, 4765–4774.

Maldonado, E. S., Shihab, E., & Tsantalis, N. (2017). Using natural language processing to automatically detect self-admitted technical debt. *IEEE Transactions on Software Engineering*, 43(11), 1044–1062. https://doi.org/10.1109/TSE.2017.2654244
...
Mockus, A., & Votta, L. G. (2000). Identifying reasons for software changes using historic databases. *Proceedings of the 16th International Conference on Software Maintenance*, 120–130. https://doi.org/10.1109/ICSM.2000.883028

Nagappan, N., Ball, T., & Zeller, A. (2006). Mining metrics to predict component failures. *Proceedings of the 28th International Conference on Software Engineering*, 452–461. https://doi.org/10.1145/1134285.1134349
...
Potdar, A., & Shihab, E. (2014). An exploratory study on self-admitted technical debt. *Proceedings of the 30th IEEE International Conference on Software Maintenance and Evolution*, 91–100. https://doi.org/10.1109/ICSME.2014.31

Saito, T., & Rehmsmeier, M. (2015). The precision-recall plot is more informative than the ROC plot when evaluating binary classifiers on imbalanced datasets. *PLOS ONE*, 10(3), e0118432. https://doi.org/10.1371/journal.pone.0118432

Schutz, A., & Plosch, R. (2023). Bridging static analysis findings and failure evidence in technical debt ground truth construction. *Information and Software Technology*, 159, 107197.

Sliwerski, J., Zimmermann, T., & Zeller, A. (2005). When do changes induce fixes? *Proceedings of the 2005 International Workshop on Mining Software Repositories*, 1–5. https://doi.org/10.1145/1083142.1083147

Tantithamthavorn, C., Hassan, A. E., & Matsumoto, K. (2017). The impact of class rebalancing techniques on the performance and interpretation of defect prediction models. *IEEE Transactions on Software Engineering*, 46(11), 1200–1219. https://doi.org/10.1109/TSE.2018.2876537

Tsoukalas, D., Jankovic, M., Siavvas, M., Kehagias, D., Chatzigeorgiou, A., & Tzovaras, D. (2020). On the applicability of time series models for technical debt forecasting. *Journal of Systems and Software*, 168, 110657. https://doi.org/10.1016/j.jss.2020.110657

Turhan, B., Menzies, T., Bener, A. B., & Di Stefano, J. (2009). On the relative value of cross-company and within-company data for defect prediction. *Empirical Software Engineering*, 14(5), 540–578. https://doi.org/10.1007/s10664-008-9103-7
...
The empirical work in this thesis uses the Technical Debt Dataset v2.0 (Lenarduzzi et al., 2019), a curated benchmark of Apache Java open-source projects.
