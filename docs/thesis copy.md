...
The gap addressed by this work is the empirical comparison, on a single citable corpus and under three labelling variants, of which feature family carries the dominant predictive signal: static SonarQube measures, historical Git process metrics, project-level context, the newer co-change graph features, or pre-snapshot defect signals.
...
1. **Consequence-oriented (primary).** Within each project, three forward-looking signals are computed inside the observation window: the number of bug-fix commits affecting the basename (`bugfix_commits_future`), the future code churn (`future_churn`, the sum of lines added and lines removed), and the number of SZZ-derived fault-fixing events touching the basename (`szz_defects_future`). Bug-fix commits are detected by a case-insensitive regex on commit messages over the keywords *fix*, *bug*, *defect*, *error*, *patch*, *resolve*, *issue \#NNN*, and *close \#NNN*, following the long-standing convention of Mockus and Votta [23]. Each signal is min-max normalised within each project before being combined into a weighted risk score using weights 0.5, 0.3, and 0.2 respectively, reflecting the proposal's emphasis on bug-fix frequency. The top twenty per cent of files by score are flagged as high-risk.
...
**Construct validity.** The unit of analysis is the *file basename* within each project rather than the full repository-relative path, forced by an asymmetry in the dataset where `GIT_COMMITS_CHANGES.FILE` stores basenames in eighteen of twenty-two projects (see Appendix B for the full diagnosis). The median basename collision rate within a project is approximately thirty-five per cent. Aggregating SonarQube issues to basename granularity preserves all information at the granularity available, but intra-basename file confusion is a residual source of label and feature noise. The consequence label's weight choice (0.5 / 0.3 / 0.2) and percentile threshold (top 20 per cent) are explicit modelling decisions; their robustness is demonstrated by the sensitivity grid in Section 4.6, which shows ROC-AUC stable across the 3 x 3 parameter sweep.
...
**Per RQ1**, the three operational definitions of high-risk technical debt - consequence, severity, and SZZ - identify largely disjoint sets of files. Pairwise Cohen's kappa lies in [0.05, 0.21] and pairwise Jaccard does not exceed 0.18. The choice of operational definition therefore fundamentally changes which files get prioritised, and any technical-debt benchmark that reports a single labelling family is implicitly choosing one *kind* of debt.

**Per RQ2**, machine-learning models can predict each variant with per-regime quality consistent with the literature. LightGBM is the strongest model on every variant. Within-project ten-fold F1 is 0.606 (consequence), 0.716 (severity), 0.324 (SZZ). Cross-project LOPO F1 is 0.401, 0.602, 0.107. Per-project temporal T1-to-T2 F1 on the consequence variant is 0.301, with CE@20 = 0.475. The cross-project CE@20 = 0.485 on the consequence variant is the headline practical number: inspecting the top twenty per cent of files in an unseen project recovers about half of the files that will impose maintenance burden in the next six months.
...
1. **The first quantitative comparison on a citable corpus** of three labelling variants on the same files. The pairwise Cohen's kappa and Jaccard tables (Table 4.3) provide the evidence that the choice of definition matters.
...
1. **Use the consequence-oriented model as the primary risk-ranking tool.** The cross-project CE@20 of 0.485 is in the practically usable range for prioritisation, and the model does not require per-project retraining provided the project lies in or near the Apache Java distribution.
...
4. **Alternative consequence formulae and labelling-as-learning.** The fixed weights (0.5 / 0.3 / 0.2) could themselves be learned from data, for example by treating the weights as additional Optuna search dimensions optimised against a downstream cost function (e.g., the integral of the cost-effectiveness curve). This would close the loop between labelling and prediction within a single optimisation framework.
