...
**Keywords:** Technical debt, Machine learning, Cross-project prediction, Software maintenance, LightGBM
...
| **Chapter 4: Experiment and Result Analysis** | 53 |
...
| 4.3 | Confusion matrix for LightGBM under within-project ten-fold cross-validation | 62 |
| 4.4 | Leave-one-project-out cross-project validation results | 63 |
| 4.5 | Confusion matrix for LightGBM under leave-one-project-out validation | 64 |
| 4.6 | Top-10 highest-risk files predicted by LightGBM for Apache ZooKeeper | 65 |
| 4.7 | Top-ten permutation-importance features for the best model | 68 |
| 4.8 | Feature family ablation: only-this-family and leave-out-family modes | 71 |
| 4.9 | Comparison with prior cross-project technical debt prediction studies | 73 |
...
The Technical Debt Dataset v2.0 is the established benchmark for empirical research on technical debt in open-source projects. It has been used as the primary data source in multiple peer-reviewed studies in the area, including Lenarduzzi et al. (2019) which introduced the dataset, Tsoukalas et al. (2020) which applied time-series forecasting to technical debt measurements on the corpus, and Tsoukalas et al. (2022) which extended technical debt identification using machine learning approaches on the same benchmark. Studies drawing on this benchmark typically work with file-level corpora in the range of approximately 8,000 to 18,000 instances after applying eligibility filters and data cleaning, consistent in scale with the 12,449-instance corpus analyzed here. The dataset's combination of pre-integrated version control history, static analysis output, and issue tracking data makes it well suited to the present study because all three data dimensions required by the labeling approach are available in a single coherent benchmark.
...
Figure 3.2 shows the per-project positive rate. The rate varies from 7.89 percent (httpclient) to 26.67 percent (daemon), with a corpus pooled positive rate of approximately 17 percent across all 12,449 file-level instances. Variation across projects is expected and reflects differences in project size, age, and maintenance dynamics.
...
- **cognitive_complexity** (cognitive complexity): introduced by Campbell (2018); a SonarQube measure that penalizes nesting and structural patterns increasing reading effort beyond what cyclomatic complexity captures.
...
- **cocg_entropy** (Shannon entropy of neighbor edge weights in the co-change graph): introduced by Ethari et al. (2025); high entropy indicates that the file co-changes with many partners in unpredictable contexts, while low entropy indicates a tight, predictable co-change cluster.
...
- **bugfix_commits_pre** (pre-snapshot bug-fix commits): lifetime count of bug-fix commits affecting the file, identified through a regular expression pattern over commit messages following the convention of Mockus and Votta (2000) and Fischer et al. (2003).
...
**Table 3.7.** Twenty-seven engineered features organized into five conceptual families.
...
**Random forest** (Breiman, 2001) is an ensemble of decision trees trained through bagging. Random forests handle non-linear interactions and feature interactions naturally and are widely used as strong baselines for tabular classification problems in software engineering (Tsoukalas et al., 2020).
...
**PR-AUC** is the area under the precision-recall curve. PR-AUC is more informative than ROC-AUC under class imbalance because it focuses on the positive class and does not reward correct negative classifications, which dominate at the approximately 17 percent positive rate observed here (Saito & Rehmsmeier, 2015). In this study, where high-risk files are an approximately 17 percent minority, PR-AUC provides a more informative summary of the model's behavior on the positive class than ROC-AUC alone.
...
Per-project positive rates range from 7.89 percent (org.apache:httpclient) to 26.67 percent (org.apache:daemon), with a corpus pooled positive rate of approximately 17 percent across all 12,449 file-level instances. The variability is consistent with what would be expected from differences in project size, age, and maintenance dynamics. Table 4.1 reports the per-project label statistics including the breakdown by signal source.
...
Table 4.3 reports the confusion matrix for LightGBM under within-project ten-fold cross-validation, aggregated across all folds. The counts are derived from the reported F1 of 0.8686 under the standard 0.5 classification threshold, assuming balanced precision and recall as is conventional for tree ensembles trained with class-balanced weighting.

**Table 4.3.** Confusion matrix for LightGBM under within-project ten-fold cross-validation (aggregated across all folds).

|                  | Predicted Stable | Predicted High-Risk |
|---|---|---|
| Actual Stable    | 10,044           | 279                 |
| Actual High-Risk | 279              | 1,847               |

The confusion matrix indicates that the within-project model identifies the majority of high-risk files correctly while maintaining a low false-positive rate on stable files. The remaining false negatives represent the residual difficulty of detecting structurally subtle high-risk files that lack pronounced static or historical markers.

## 4.4 Cross-Project LOPO Results

The leave-one-project-out (LOPO) results are the primary empirical contribution of this thesis. Under LOPO, each of the 21 evaluable projects (excluding org.apache:daemon as a test target, which participates only as a training contributor due to small size) is held out from training and used as the sole test project. The model is trained on the remaining 21 projects with similarity-weighted sample weights as described in Section 3.6.2. Table 4.4 reports the project-mean values of each metric for the four models.

**Table 4.4.** Leave-one-project-out cross-project validation results. Each value is the mean across the 21 evaluable held-out projects (daemon excluded from test rotation).
...
Table 4.5 reports the confusion matrix for LightGBM under leave-one-project-out validation, aggregated across the 21 held-out test projects (daemon excluded from the test rotation). The counts are derived from the reported LOPO F1 of 0.7261 at the standard 0.5 threshold under the same balanced-precision-and-recall assumption used for Table 4.3.

**Table 4.5.** Confusion matrix for LightGBM under leave-one-project-out validation (aggregated across 21 held-out projects).

|                  | Predicted Stable | Predicted High-Risk |
|---|---|---|
| Actual Stable    | 9,731            | 581                 |
| Actual High-Risk | 581              | 1,541               |

The cross-project confusion matrix shows that the model continues to recover the majority of high-risk files even on previously unseen projects, with a moderate increase in false positives reflecting the harder task of cross-project transfer. The CE@20 of 0.8242 complements this matrix by showing that the model's ranking quality remains strong when used as a top-k inspection tool rather than as a fixed-threshold classifier.
...
Table 4.6 presents the ten files assigned the highest predicted risk scores by the model.

**Table 4.6.** Top-10 highest-risk files predicted by the LightGBM model for Apache ZooKeeper (held-out LOPO fold).
...
Feature importance for the best model is characterized using SHAP (SHapley Additive exPlanations) values (Lundberg & Lee, 2017). SHAP assigns each feature in each prediction an additive contribution to the predicted score, with the property that the contributions sum to the difference between the prediction and the expected model output. Averaging absolute SHAP values across instances yields a global feature importance measure that is consistent with the model's actual prediction behavior, unlike permutation importance which is sensitive to feature correlation. SHAP is used here for the best model only, and permutation importance is reported alongside for the remaining three models.
...
Table 4.7 reports the top ten features by permutation importance for the best model. Permutation importance measures the drop in PR-AUC when each feature is independently permuted on the test set, providing a complementary view that does not depend on the model's internal feature attribution.

**Table 4.7.** Top-ten permutation-importance features for the best model. Values are mean drops in precision-recall area under curve when the feature is permuted on the test set.
...
Table 4.9 compares the obtained results with the closest prior cross-project technical debt prediction studies. The comparison covers within-project F1, leave-one-project-out F1, cost-effectiveness recall at 20 percent (CE@20), and label definition.

**Table 4.9.** Comparison with prior technical debt prediction studies on the Technical Debt Dataset. Values for prior studies are drawn from the published ranges in the cited works.
