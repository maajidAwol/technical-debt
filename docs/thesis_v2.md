<!-- Revised from thesis.md on 2026-05-11. Original preserved. -->
...
Build instructions
==================
This document is the revised Markdown source for the thesis. The original thesis.md
is preserved unchanged. To produce the Word version expected by AASTU, run from the
project root:
...
All numeric content, tables, and figures referenced below are reproducible by
re-running the analysis pipeline; this thesis documents those results and does not
modify them.
-->
...
The three labelling variants identify largely disjoint file sets, with pairwise Cohen's kappa between five and twenty-one per cent, providing the first quantitative evidence on this corpus that the choice of operational definition fundamentally changes which files get prioritised. LightGBM is the strongest model on every variant. Within-project F1 reaches 0.606 for the consequence variant and 0.324 for the SZZ variant; cross-project F1 reaches 0.401 and 0.107 respectively. The cost-effectiveness at the top twenty per cent budget is 0.485 for the consequence variant under cross-project validation, meaning that inspecting the highest-ranked twenty per cent of files in an unseen project recovers about half of the truly burdensome modules. The severity control achieves within-project F1 of 0.716 and cross-project F1 of 0.602, confirming its near-tautological structure rather than constituting an independent prediction result. Consequence and SZZ rely on historical process and graph features, whereas severity is best predicted by static SonarQube features alone, confirming the expected tautological dependency. The pipeline is fully reproducible end-to-end on a 22-project Apache corpus.
...
1. Construct a file-level dataset from selected open-source software projects, suitable for cross-project analysis at a fixed temporal snapshot per project.
2. Extract and engineer predictive features by combining static code metrics with historical evolution metrics from repository history, while enforcing strict temporal separation between feature extraction and label derivation.
3. Train and compare machine-learning models for high-risk technical-debt prediction and evaluate generalisability using stratified within-project, leave-one-project-out, and temporal cross-validation regimes, supported by confidence intervals and statistical significance testing.
4. Identify key predictors of high-risk technical debt through feature importance analysis and error examination, and assess the generalisability of predictions across projects and time periods.
...
This chapter describes the experimental pipeline that produces every quantitative result reported in Chapter 4. The pipeline is fully reproducible by installing pinned dependencies and running the analysis end-to-end. Each subsection corresponds to a specific stage or to a cross-cutting concern that affects multiple stages.

## 3.1 Research Design

The research follows an experimental, quantitative, supervised-classification design built around three operational definitions of high-risk technical debt that are evaluated on identical feature vectors. The unit of analysis is a file within each project at a fixed per-project snapshot date. The pipeline operates in stages: data preparation, labelling, feature engineering, model training, evaluation, and interpretation. All intermediate results are persisted and reproducible by re-running the analysis pipeline end-to-end.

The implemented methodology extends the approved research proposal in seven respects, each closing an explicit gap identified during the experimental phase: (1) co-change graph centrality features (architectural connectivity measures) implemented via igraph realise the proposal's stated commitment to graph-based and social-network style metrics [7, 8]; (2) pre-snapshot defect signals extend the feature space beyond static and historical metrics; (3) all candidate machine-learning classifiers mentioned in the proposal are evaluated and compared; (4) hyperparameter tuning via Optuna's tree-structured Parzen estimator [17] is applied; (5) probability calibration using Platt scaling and isotonic regression to improve confidence estimates is added; (6) resampling strategies including SMOTE are compared explicitly; and (7) temporal validation provides a forward-time sanity check beyond the cross-project validation regime. Feature importance is analysed via SHAP values to provide model-agnostic interpretation. The core research design—consequence-oriented labelling philosophy, cross-project validation emphasis, and multi-family feature engineering—remains as approved. Each extension is documented chronologically in Appendix B.
...
| Co-change graph centralities | Per-project undirected weighted graph built from pre-`t` Git change cliques | `cocg_degree`, `cocg_strength_sum`, `cocg_strength_mean`, `cocg_strength_max`, `cocg_betweenness`, `cocg_closeness`, `cocg_clustering_coef`, `cocg_pagerank`, `cocg_neighbour_count_30d`, `cocg_neighbour_count_90d` | Architectural fragility signals motivated by Jiang et al. [7], [8] and D'Ambros et al. Computed using the igraph library for runtime efficiency, with numerical parity verified against networkx to guarantee reproducibility. |
...
Stage 7b runs hyperparameter search per (variant, model) using the tree-structured Parzen estimator (TPE) implemented in Optuna [17], a Bayesian-optimisation framework. The objective is mean precision-recall area under the curve on an inner stratified five-fold split, following the recommendation of Saito and Rehmsmeier [18] for imbalanced classification; thirty trials per configuration ensure thorough exploration within computational constraints. SVM is excluded from tuning because its RBF kernel cost makes thirty-trial inner cross-validation infeasible on the corpus. Each tuned configuration is then re-evaluated on the canonical outer ten-fold cross-validation, and both results are reported in Section 4.9. Selected hyperparameters per (variant, model) are persisted for auditability (see Appendix C).

## 3.9 Probability Calibration

Stage 7c applies probability calibration to each tuned classifier using Platt scaling (sigmoid) and isotonic regression in parallel, with an inner stratified five-fold split disjoint from the outer evaluation fold [19]. Three calibration metrics are reported: the Brier score, the negative log-likelihood (NLL), and the expected calibration error (ECE) computed via equal-frequency binning. The uncalibrated baseline is included in the same table for reference. Reliability diagrams (Fig. 4.10) visualise the calibration shape per (variant, model, method).
...
The within-project and LOPO summaries are accompanied by ten-thousand-resample bootstrap percentile confidence intervals at the 95 per cent level. Pairwise Wilcoxon signed-rank tests are run on per-fold F1 and PR-AUC across all model pairs within each variant, following Demsar's recommendation. Bonferroni correction is applied per metric, with fifteen pairs for the six within-project models and ten pairs for the five LOPO models. Both raw and Bonferroni-corrected p-values are reported in the detailed results (see Appendix C).

## 3.12 Implementation, Reproducibility, and Pipeline-Stages Mapping

The entire pipeline is implemented in Python using standard open-source libraries for data manipulation, machine learning, analysis, and visualization. The pipeline is fully reproducible: every numeric result, table, and figure can be regenerated by re-running the analysis end-to-end from publicly available data sources.

**Reproducibility Pathways:**

Two interactive pathways are provided for readers and practitioners:

1. **Quick-start demo**: A demo notebook reproduces the core analyses on a representative subset of projects in approximately ten minutes on freely available cloud infrastructure (Google Colab free tier).
2. **Complete analysis**: A comprehensive notebook reproduces every result in this thesis end-to-end on cloud infrastructure with standard hardware specifications (Google Colab Pro or equivalent).

Both notebooks include step-by-step documentation, are version-controlled, and require only standard Python package installation. Intermediate results are cached and persisted, enabling partial reruns and targeted re-analyses.

**Pipeline Structure:**

The analysis pipeline comprises ten logical stages: (1) data source inspection and schema validation, (2) project eligibility filtering, (3) snapshot selection and metadata extraction, (4) labelling under the three labelling variants, (5) feature engineering and representation, (6) dataset assembly with leakage auditing, (7) model training and evaluation, (8) cross-project generalisation assessment, (9) sensitivity and ablation analysis, and (10) results synthesis and reporting. Each stage produces persisted, inspectable outputs that feed into downstream stages. The pipeline is deterministic (all random seeds are fixed) and supports reproducible re-execution from the original data sources.

\newpage

# Chapter 4 Results

This chapter reports the empirical results of the pipeline. All numbers, tables, and figures are reproducible by running the pipeline end-to-end from the public data sources described in Section 3.2. The results section is organised by research question, validation regime, and analysis dimension.
...
Inferential rigour was further strengthened by adding 10,000-resample bootstrap percentile confidence intervals and Bonferroni-corrected paired Wilcoxon signed-rank tests on per-fold metrics. Whether the headline rankings survive significance correction is now empirically answerable from detailed results tables rather than inferable from point estimates.
...
The overarching contribution of this thesis is a shift in the technical debt prediction target from tool-defined debt *presence* to empirically grounded future *maintenance burden*, demonstrated on a multi-project citable corpus with a fully reproducible end-to-end pipeline. The four specific contributions are:

1. **The first quantitative comparison of three labelling variants** on identical feature vectors: consequence-oriented (based on future maintenance signals), severity-based (tool-defined), and defect-history-based (based on prior fault patterns). The pairwise label agreement metrics provide empirical evidence that the choice of operational definition fundamentally changes which files are prioritised. A secondary finding is that severity-derived labels carry an inherent tautological dependency with standard code-quality metrics, meaning that prior studies using severity as ground truth are benchmarking tool consistency rather than future maintenance risk—a methodological caution for the field.
2. **A consequence-oriented labelling protocol with a fully audited leakage discipline.** The pipeline ensures that no features that would trivially reconstruct the labels are retained in the training data, maintaining strict temporal separation between feature extraction and label derivation.
3. **An igraph-backed implementation of co-change graph centrality features** for the technical-debt prediction setting, with numerical equivalence guarantees against networkx and optimisation for large-scale projects. This implementation is fifty times faster than naive graph-traversal approaches and demonstrates practical efficiency suitable for projects with thousands of files.
4. **A complete statistical inference layer** combining resampling-based confidence intervals, multiple-comparison correction, hyperparameter tuning, probability calibration, and model-agnostic feature importance analysis. These components are integrated into a single reproducible pipeline that can be re-executed to regenerate every numeric output.
...
**A.1 Software environment.** Python 3.12 with pinned dependencies:
...
**A.2 Data.** Download the Technical Debt Dataset v2.0 SQLite file (approximately 1.54 gigabytes) from the official repository. Place it in the data directory as specified in configuration.

**A.3 Configuration knobs.** All configuration is centralised in the pipeline configuration file. The principal knobs are:
...
Each stage is independently runnable via command-line flags; interruptions can be resumed from a specific stage without recomputing prior stages. Logs are written to the configured log directory.

**A.5 Google Colab.** Two Jupyter notebooks materialise the same pipeline on Google's hosted runtime with optimised data handling and tier-aware parallelism:
...
**B.6 Pipeline performance overhaul (2026-04-29).** Stage 5 (feature engineering) was identified as a bottleneck due to graph centrality computation cost. The implementation was migrated to igraph for betweenness, closeness, clustering, and PageRank, with numerical parity verification against NetworkX. Project-level parallelism was added to computation stages. Total pipeline runtime was reduced from approximately ten hours to approximately three to four hours on standard hardware.

**B.7 Verification.** Comprehensive tests on all newly added modules confirmed correct implementation and clean dependency imports. Parity verification ensures the igraph-based graph feature computation remains numerically equivalent to the NetworkX reference implementation across all test projects.

\newpage

# Appendix C. Tuned Hyperparameters

The Optuna 30-trial PR-AUC tuning per (variant, model) cell selected the configurations below, which are reproducible by re-running the tuning stage.
...
The full versions of the most heavily abridged tables in Chapter 4 are reproducible by re-running the analysis pipeline and are detailed in the supplementary results. They are referenced rather than reproduced here to keep the body of the thesis at a manageable length.

**Supplementary Results** include detailed output for feature ablation analysis, pairwise statistical significance testing (within-project and cross-project), calibration metrics, per-project LOPO results, temporal validation results, bootstrap confidence intervals, complete SHAP and permutation importance values, resampling comparisons, sensitivity grid analysis, and per-project error rates. All outputs are deterministically reproducible by re-running the pipeline from the data sources specified in Section 3.2.
